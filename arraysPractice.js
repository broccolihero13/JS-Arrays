...
function reversedLooper(letters) {
  for(var i = 1; i <= letters.length; i++){
    alert(letters[letters.length - i])
  }
}
...
function finder(array){
  var checker = false;
  var numberToCheck = getRandomArbitrary();
...
//array after adding ten to each item in numbers. *Verify your answer is correct. --> [15, 19, 26, 29, 35, 44, 58]
  ...
  //Code Here
function addTen(arrayOfNumbers){
  var arrayPlus10 = [];
  for(var i = 0; i < arrayOfNumbers.length; i++){
    arrayPlus10.push(parseInt(arrayOfNumbers[i]) + 10);
  }
  return arrayPlus10;
}
...
var user2 = {
  ...
  email: 'fred@inthetownofbedrock.com',
...
var user3 = {
  name: 'Barney Rubble',
  email: 'barney@inthetownofbedrock.com',
  password: 'weoughtadosomethingfred',
...
for(var i = 0; i < users.length; i++){
  if(users[i].username === "infiniateLoop"){
    users.splice(i,1);
  }
}
